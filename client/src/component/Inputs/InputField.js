...
const InputField = ({value,setValue,nameKey,type,invalidFields,setInvalidFields}) => {
  ...
  return (
    <div className='w-full flex flex-col relative mb-2'>
        {value.trim() !== '' && <label htmlFor={nameKey} className='text-[10px] animate-slide-top-sm absolute top-0 left-[8px] block bg-white px-1'>{nameKey?.slice(0,1).toUpperCase() + nameKey?.slice(1)}</label>}
        <input 
        type={type || 'text'} 
